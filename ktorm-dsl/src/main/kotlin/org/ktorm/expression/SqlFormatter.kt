 ...
 *
 * @property dsl the current database object used to obtain metadata such as identifier quote string.
...
public abstract class SqlFormatter(
    ...
    protected fun writeKeyword(keyword: String) {
        when (dsl.generateSqlInUpperCase) {
            true -> {
            ...
            null -> {
                if (dsl.supportsMixedCaseIdentifiers || !dsl.storesLowerCaseIdentifiers) {
                    _builder.append(keyword.uppercase())
                } else {
                    _builder.append(keyword.lowercase())
                }
    ...
    protected open fun shouldQuote(identifier: String): Boolean {
        if (dsl.alwaysQuoteIdentifiers) {
            return true
        }
        if (!identifier.isIdentifier) {
            return true
        }
        if (identifier.uppercase() in dsl.keywords) {
            return true
        }
        if (identifier.isMixedCase
            && !dsl.supportsMixedCaseIdentifiers
            && dsl.supportsMixedCaseQuotedIdentifiers
        ) {
            return true
        }
        return false
    ...
    protected val String.quoted: String get() {
        if (shouldQuote(this)) {
            if (dsl.supportsMixedCaseQuotedIdentifiers) {
                return "${dsl.identifierQuoteString}${this}${dsl.identifierQuoteString}"
            } else {
                if (dsl.storesUpperCaseQuotedIdentifiers) {
                    return "${dsl.identifierQuoteString}${this.uppercase()}${dsl.identifierQuoteString}"
                }
                if (dsl.storesLowerCaseQuotedIdentifiers) {
                    return "${dsl.identifierQuoteString}${this.lowercase()}${dsl.identifierQuoteString}"
                }
                if (dsl.storesMixedCaseQuotedIdentifiers) {
                    return "${dsl.identifierQuoteString}${this}${dsl.identifierQuoteString}"
                }
            ...
            if (dsl.supportsMixedCaseIdentifiers) {
                return this
            } else {
                if (dsl.storesUpperCaseIdentifiers) {
                    return this.uppercase()
                }
                if (dsl.storesLowerCaseIdentifiers) {
                    return this.lowercase()
                }
                if (dsl.storesMixedCaseIdentifiers) {
                    return this
                }
                // Should never happen, but it's still needed as some database drivers are not implemented correctly.
                return this
    ...
    protected val Char.isIdentifierStart: Boolean get() {
        ...
        if (this in dsl.extraNameCharacters) {
            return true
        }
        return false
    }
